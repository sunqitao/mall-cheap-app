	...
	<view class="good-list">
		<view v-for="(item, index) in list" :key="index" @click="navToDetailPage(item)" class="good-item">
			<view class="image-wrapper">
				<image :src="item.picUrl" lazy-load mode="aspectFill"></image>
			</view>
			<view class="product-title">
				<!--#ifndef MP-WEIXIN || MP-QQ-->
				<text :class="{'tm':item.tmall===1,'tb':item.tmall===0}"></text>
				<!-- #endif -->
				<text class="title-text clamp">{{item.name}}</text>
			</view>
			<view class="product-price">
				<text style="font-size: 20upx;">{{item.yijuhua}}{{item.preferential}}</text>
			...
			<!-- <view style="font-size: 8px; font-weight: 340;  color: #fa436a;">{{item.shopName}}</view> -->
			<view class="salse">
				<view class="text">开抢时间:{{item.startTime}}</view>
	...
	export default {
		...
		methods: {
			...
			navToDetailPage(item) {
				uni.navigateTo({
					url: `/pages/product/product?goodsId=${item.itemId}`
				})
			}
		}
	}
